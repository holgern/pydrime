"""
Benchmark script for LOCAL_BACKUP sync mode.

LOCAL_BACKUP mode: Only upload data to the cloud, never delete anything
...
3. Delete file: Delete local file, verify it is NOT deleted from cloud
   (LOCAL_BACKUP does not delete remote files)
...
def run_sync_command(
    ...
    Args:
        sync_pair: Sync pair string (e.g., "/local:localBackup:/remote")
...
def test_initial_state(
    ...
    # Sync using localBackup mode
    sync_pair = f"{local_dir}:localBackup:/{remote_folder}"
    ...
    # Verify no downloads (LOCAL_BACKUP never downloads)
...
def test_add_file(local_dir: Path, remote_folder: str) -> bool:
    ...
    LOCAL_BACKUP should upload the new file.
    """
    print("\n" + "=" * 80)
...
def test_delete_file(local_dir: Path, remote_folder: str) -> bool:
    """Test 3: Delete file - delete local file, verify it is NOT deleted from cloud.

    LOCAL_BACKUP does NOT delete remote files (allows_remote_delete=False).
    The file should remain in cloud even after local deletion.
    """
    print("\n" + "=" * 80)
    print("TEST 3: DELETE FILE - Delete local file (should NOT delete from cloud)")
    print("=" * 80)
    ...
    # Verify 0 files were deleted from remote (LOCAL_BACKUP does NOT delete remote)
    if stats["deleted_remote"] != 0:
        print(
            f"[FAIL] Expected 0 remote deletions (backup mode), "
            f"got {stats['deleted_remote']}"
        )
        return False

    print("[PASS] Delete file: remote file was preserved (backup behavior confirmed)")
    return True
...
def test_modify_file(local_dir: Path, remote_folder: str) -> bool:
    ...
    LOCAL_BACKUP should upload the modified file.
    """
    print("\n" + "=" * 80)
...
def main() -> None:
    """Main benchmark function for LOCAL_BACKUP sync mode."""
    print("\n" + "=" * 80)
    print("PYDRIME BENCHMARK: LOCAL_BACKUP SYNC MODE")
    print("=" * 80)
    print("\nMode description: Only upload data to the cloud, never delete anything")
    print("or act on cloud changes.")
    print("Renaming & moving is transferred to the cloud, but not local deletions.")

    # Generate unique folder name
    test_uuid = str(uuid.uuid4())
    remote_folder = f"benchmark_lb_{test_uuid[:8]}"
    base_dir = Path.cwd() / f"benchmark_temp_lb_{test_uuid[:8]}"
    ...
    try:
        ...
        # Test 3: Delete file (should NOT delete from cloud)
        ...
        # All tests passed
        print("\n" + "=" * 80)
        print("ALL TESTS PASSED FOR LOCAL_BACKUP MODE")
        print("=" * 80)
        print("\nSummary:")
        print("  + Initial state: Local files uploaded to cloud")
        print("  + Add file: New file uploaded to cloud")
        print("  + Delete file: Remote file preserved (backup behavior)")
