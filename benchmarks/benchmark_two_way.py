...
TWO_WAY mode: Mirror every action in both directions.
...
def test_initial_state(local_dir: Path, remote_folder: str) -> bool:
    ...
    TWO_WAY should upload all local files to cloud.
    """
    print("\n" + "=" * 80)
...
def test_add_file(local_dir: Path, remote_folder: str) -> bool:
    ...
    TWO_WAY should upload the new file.
    """
    print("\n" + "=" * 80)
...
def test_delete_file(local_dir: Path, remote_folder: str) -> bool:
    ...
    TWO_WAY should delete the file from cloud (allows_remote_delete=True).
    """
    print("\n" + "=" * 80)
    ...
    # Verify 1 file was deleted from remote
...
def test_modify_file(local_dir: Path, remote_folder: str) -> bool:
    ...
    TWO_WAY should upload the modified file.
    """
    print("\n" + "=" * 80)
...
def main() -> None:
    """Main benchmark function for TWO_WAY sync mode."""
    print("\n" + "=" * 80)
    print("PYDRIME BENCHMARK: TWO_WAY SYNC MODE")
    print("=" * 80)
    print("\nMode description: Mirror every action in both directions.")
    print("Renaming, deleting & moving is applied to both sides.")

    # Generate unique folder name
    test_uuid = str(uuid.uuid4())
    remote_folder = f"benchmark_twoway_{test_uuid[:8]}"
    base_dir = Path.cwd() / f"benchmark_temp_twoway_{test_uuid[:8]}"
    ...
    try:
        ...
        # All tests passed
        print("\n" + "=" * 80)
        print("ALL TESTS PASSED FOR TWO_WAY MODE")
        print("=" * 80)
        print("\nSummary:")
