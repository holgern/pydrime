"""
Benchmark script for CLOUD_BACKUP sync mode.

CLOUD_BACKUP mode: Only download data from the cloud, never delete anything
or act on local changes.
Renaming & moving is transferred to the local side, but not cloud deletions.

Tests:
1. Initial state: Create files in cloud, then sync to download locally
2. Add file: Add new file to cloud, verify it downloads locally
3. Delete file: Delete file from cloud, verify local file is NOT deleted
   (CLOUD_BACKUP does not delete local files)
...
def run_sync_command(
    ...
    Args:
        sync_pair: Sync pair string (e.g., "/local:cloudBackup:/remote")
        workers: Number of parallel workers
        batch_size: Number of files to process per batch
        start_delay: Delay between starting worker processes
...
def test_initial_state(local_dir: Path, staging_dir: Path, remote_folder: str) -> bool:
    ...
    CLOUD_BACKUP should download all cloud files to local.
    """
    print("\n" + "=" * 80)
    ...
    sync_pair = f"{local_dir}:cloudBackup:/{remote_folder}"
    ...
    # Verify no uploads (CLOUD_BACKUP never uploads)
    if stats["uploaded"] != 0:
...
def test_add_file(local_dir: Path, staging_dir: Path, remote_folder: str) -> bool:
    ...
    CLOUD_BACKUP should download the new file.
    """
    print("\n" + "=" * 80)
...
def test_delete_file(local_dir: Path, remote_folder: str) -> bool:
    """Test 3: Delete file - delete file from cloud, verify local file is NOT deleted.

    CLOUD_BACKUP does NOT delete local files (allows_local_delete=False).
    The file should remain locally even after cloud deletion.
    """
    print("\n" + "=" * 80)
    print("TEST 3: DELETE FILE - Delete file from cloud (should NOT delete locally)")
    print("=" * 80)

    # Record which file we expect to keep locally
    local_file = local_dir / "test_file_000.txt"
    file_exists_before = local_file.exists()
    print(f"\n[INFO] Local file exists before cloud deletion: {file_exists_before}")
    ...
    # Verify 0 files were deleted locally (CLOUD_BACKUP does NOT delete local)
    if stats["deleted_local"] != 0:
        print(
            f"[FAIL] Expected 0 local deletions (backup mode), "
            f"got {stats['deleted_local']}"
        )
        return False

    # Verify file still exists locally
    if not local_file.exists():
        print(f"[FAIL] Local file should still exist: {local_file}")
        return False

    print("[PASS] Delete file: local file was preserved (backup behavior confirmed)")
    return True
...
def test_modify_file(local_dir: Path, staging_dir: Path, remote_folder: str) -> bool:
    ...
    CLOUD_BACKUP should download the modified file.
    """
    print("\n" + "=" * 80)
...
def main() -> None:
    """Main benchmark function for CLOUD_BACKUP sync mode."""
    print("\n" + "=" * 80)
    print("PYDRIME BENCHMARK: CLOUD_BACKUP SYNC MODE")
    print("=" * 80)
    print(
        "\nMode description: Only download data from the cloud, never delete anything"
    )
    print("or act on local changes.")
    print(
        "Renaming & moving is transferred to the local side, but not cloud deletions."
    )
    ...
    remote_folder = f"benchmark_cb_{test_uuid[:8]}"
    base_dir = Path.cwd() / f"benchmark_temp_cb_{test_uuid[:8]}"
    ...
    try:
        ...
        # Test 3: Delete file (should NOT delete locally)
