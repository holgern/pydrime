...
def s3(
    ...
    # Get config and workspace
    if workspace is None:
        workspace = config.get_default_workspace()
        if workspace is None:
            out.print(
                "[red]Error:[/red] No workspace specified and no default "
                "workspace configured.\n"
                "\n"
                "Either:\n"
                "  - Set a default workspace: [cyan]pydrime "
                "set-default-workspace <id>[/cyan]\n"
                "  - Specify workspace explicitly: [cyan]pydrime server s3 "
                "--workspace <id>[/cyan]"
            )
            ctx.exit(1)
        workspace = int(workspace)

    # Get Drime credentials
    username = config.get("username")
    password = config.get("password")

    if not username or not password:
        out.print(
            "[red]Error:[/red] Drime credentials not found in pydrime config.\n"
            "\n"
            "Please run: [cyan]pydrime login[/cyan]"
        )
        ctx.exit(1)

    out.print("[bold blue]Starting S3 server...[/bold blue]")
    out.print(f"  Workspace: [cyan]{workspace}[/cyan]")
    if root_folder:
        out.print(f"  Root folder: [cyan]{root_folder}[/cyan]")
    out.print(f"  Listen address: [cyan]{listen}[/cyan]")

    if no_auth:
        out.print("  [yellow]Authentication: DISABLED[/yellow]")
        out.print("  [yellow]⚠️  Warning: Server allows anonymous access![/yellow]")
    else:
        out.print(f"  Access Key ID: [cyan]{access_key_id}[/cyan]")
        out.print("  [green]Authentication: ENABLED[/green]")

    out.print(f"  Region: [cyan]{region}[/cyan]")
    out.print()

    # Show connection examples
    if no_auth:
        out.print("[bold]Connect with S3 clients:[/bold]")
        out.print(
            f"  AWS CLI: [cyan]aws s3 ls --endpoint-url http://{listen} "
            "--no-sign-request[/cyan]"
        )
        out.print(
            f"  rclone:  [cyan]rclone lsd s3server: --s3-endpoint "
            f"http://{listen} --s3-no-check-bucket[/cyan]"
        )
    else:
        out.print("[bold]Connect with S3 clients:[/bold]")
        endpoint = listen if listen.startswith("http") else f"http://{listen}"
        out.print(
            f"  AWS CLI: [cyan]aws configure set aws_access_key_id "
            f"{access_key_id}[/cyan]"
        )
        out.print(
            f"           [cyan]aws configure set aws_secret_access_key "
            f"{secret_access_key}[/cyan]"
        )
        out.print(f"           [cyan]aws s3 ls --endpoint-url {endpoint}[/cyan]")
        out.print()
        out.print("  rclone config example:")
        out.print("    [cyan][s3server][/cyan]")
        out.print("    [cyan]type = s3[/cyan]")
        out.print("    [cyan]provider = Other[/cyan]")
        out.print(f"    [cyan]endpoint = {endpoint}[/cyan]")
        out.print(f"    [cyan]access_key_id = {access_key_id}[/cyan]")
        out.print(f"    [cyan]secret_access_key = {secret_access_key}[/cyan]")

    out.print()
    out.print("[yellow]Press Ctrl+C to stop the server[/yellow]")
    out.print()
    ...
    try:
        ...
        out.print(f"[red]Server error:[/red] {e}")
        if debug:
            import traceback
