...
@click.option(
    ...
    type=str,
    default=None,
    help="WebDAV username for authentication (anonymous if not set)",
)
@click.option(
    "--password",
    "-P",
    type=str,
    default=None,
    help="WebDAV password (will prompt if username is set but password is not)",
)
@click.option(
    "--readonly",
    is_flag=True,
...
@click.option(
    ...
    type=float,
    default=30.0,
    help="Directory listing cache TTL in seconds (default: 30)",
)
@click.option(
    "--max-file-size",
    type=int,
    default=500,
    help="Maximum file size in MB for uploads/downloads (default: 500)",
)
@click.option(
    "--ssl-cert",
    type=click.Path(exists=True),
...
@click.option(
    ...
    type=int,
    default=1,
    help="WsgiDAV verbosity level 0-5 (default: 1)",
)
@click.pass_context
def webdav(
    ctx: Any,
    ...
    Exposes your Drime Cloud storage over the WebDAV protocol,
    allowing you to mount it as a local drive on any operating system.
    ...
    File Size Limits:
      The --max-file-size option limits uploads and downloads to prevent
      memory issues. Default is 500 MB.

    Caching:
      Directory listings are cached for --cache-ttl seconds (default: 30)
      to reduce API calls. Set to 0 to disable caching.

    Examples:
        # Start server with defaults (localhost:8080, anonymous)
        pydrime webdav

        # Bind to all interfaces on port 8888
        pydrime webdav --host 0.0.0.0 --port 8888

        # Require authentication
        pydrime webdav --username myuser --password mypass
        ...
        pydrime webdav --cache-ttl 5

        # Larger file size limit (1 GB)
        pydrime webdav --max-file-size 1000

    Mounting the WebDAV share:
        Linux:   mount -t davfs http://localhost:8080 /mnt/drime
        macOS:   Finder -> Go -> Connect to Server -> http://localhost:8080
        Windows: net use Z: http://localhost:8080
    """
    out: OutputFormatter = ctx.obj["out"]
    ...
    if verbose < 0 or verbose > 5:
        out.error("Verbose level must be between 0 and 5")
        ctx.exit(1)

    # Prompt for password if username is set but password is not
    if username and not password:
        password = click.prompt("WebDAV password", hide_input=True)
    ...
    try:
        # Check if wsgidav and cheroot are installed
        try:
            from .webdav import run_webdav_server
        except ImportError as e:
            out.error(
                "WebDAV dependencies not installed. "
                "Install with: uv pip install wsgidav cheroot"
            )
            out.error(f"Import error: {e}")
            ctx.exit(1)
            return
        ...
        if not out.quiet:
            out.info("=" * 60)
            out.info("Drime Cloud WebDAV Server")
            out.info("=" * 60)
            ...
            out.info(f"Authentication: {auth_mode}")
            out.info(f"Mode: {'Read-only' if readonly else 'Read-write'}")
            out.info(f"Cache TTL: {cache_ttl}s")
            out.info(f"Max file size: {max_file_size} MB")
            out.info("")
            out.info("Press Ctrl+C to stop the server")
            out.info("=" * 60)
        ...
        if not out.quiet:
            out.info("WebDAV server stopped.")
